...
void FiniteStateMachine::Update(float deltaTime)
{
    ...
    //auto result = m_Transitions[m_pCurrentState];
    ...
    if (currentTransitions != m_Transitions.end())
    {
        ...
        for (auto& transitionState : currentTransitions->second)
        {
            FSMCondition* condition = transitionState.first;
            FSMState* nextState = transitionState.second;
