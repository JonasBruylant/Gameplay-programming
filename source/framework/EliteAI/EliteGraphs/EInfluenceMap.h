...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		for (const auto& currentNode : m_Nodes)
		{
			...
			for (const auto& currentConnection : GetNodeConnections(currentNode))
			{
				auto newInfluence = GetNode(currentConnection->GetTo())->GetInfluence() * expf(-currentConnection->GetCost() * m_Decay);
				if (abs(newInfluence) > abs(highestInfluence))
					highestInfluence = newInfluence;
			}
			float lerpedInfluence = Lerp(currentNode->GetInfluence(), highestInfluence, m_Momentum);
			m_InfluenceDoubleBuffer[currentNode->GetIndex()] = lerpedInfluence;
		}

		for (const auto& currentNode : m_Nodes)
		{
			m_Nodes[currentNode->GetIndex()]->SetInfluence(m_InfluenceDoubleBuffer[currentNode->GetIndex()]);
		}
