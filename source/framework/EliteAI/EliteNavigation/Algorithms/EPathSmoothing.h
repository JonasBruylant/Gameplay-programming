	...
	public:
		...
		static std::vector<Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int portalIdx = 1; portalIdx < amtPortals; ++portalIdx)
			{
				...
				if (leftLeg.Cross(newLeftLeg) <= 0) //Check if right leg is going inward
				{
			...
			vPath.emplace_back(portals[portals.size() - 1].Line.p1);
			//vPath.emplace_back(portals.end()->Line.p1);
			return vPath;
		}
	private:
		SSFA() {};
