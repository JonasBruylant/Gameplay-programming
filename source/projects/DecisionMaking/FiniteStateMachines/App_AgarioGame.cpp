...
void App_AgarioGame::Start()
{
	...
	FSMStates::SeekFoodstate* pSeekFood = new FSMStates::SeekFoodstate();

	//3. Create the conditions to transitions between those states
	FSMConditions::FoodNearbyCondition* pFoodNearby = new FSMConditions::FoodNearbyCondition();
	m_pConditions.push_back(pFoodNearby);

	FSMConditions::AreaClear* pAreaClear = new FSMConditions::AreaClear();
	m_pConditions.push_back(pAreaClear);
	...
	// pStateMachine->AddTransition(startState, toState, condition)
	...
	pStateMachine->AddTransition(pSeekFood, pWanderState, pAreaClear);
...
Blackboard* App_AgarioGame::CreateBlackboard(AgarioAgent* a)
{
	...
	pBlackboard->AddData("Enemies", &m_pAgentVec);
	//...
